...
- 桌面版 GitHub Releases（Windows / macOS / Linux，请选择 `app-v*`）：https://github.com/topcheer/PrivateClaw/releases
...
所有手工安装路径都请保留 `--dangerously-force-unsafe-install`。如果你手边已经有本地 package 目录（例如当前仓库 checkout，或者 `npx` 在本机下载出来的 package 目录），优先直接安装这个目录，因为 OpenClaw `2026.3.31` 的 archive 安装分支会把 unsafe-install override 丢掉：
...
如果你不是要安装 npm 已发布版本，而是想立刻使用 GitHub 仓库里的最新代码，直接安装本地 package 目录即可，不要先打包 archive：
...
- 桌面端产物格式：
  - Windows：`.zip`
  - Windows Store：用于手工上传到 Partner Center 的已验证未签名 `.msix`
  - macOS：`.dmg`（`x64`、`arm64`）
...
当前仓库固定使用 Flutter `3.41.6`。GitHub 的 app release workflow 会发布 Windows 桌面 ZIP 和用于手工上传到 Partner Center 的未签名 Windows Store MSIX，同时继续发布 macOS / Linux 的 `x64` 与 `arm64` 桌面产物。Linux `arm64` 使用已经验证过的 `ubuntu-24.04-arm` clone-and-bootstrap 路径，macOS 双架构仍继续走正常的预构建 SDK 路径。
