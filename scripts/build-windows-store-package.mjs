...
function resolveWindowsBuildReleaseDir(architecture) {
  return path.join(appRoot, "build", "windows", architecture, "runner", "Release");
}
...
export function main(argv = process.argv.slice(2)) {
  ...
  const expectedReleaseDir = resolveWindowsBuildReleaseDir(options.architecture);
  if (!fs.existsSync(expectedReleaseDir)) {
    if (options.architecture === "arm64") {
      const x64ReleaseDir = resolveWindowsBuildReleaseDir("x64");
      if (fs.existsSync(x64ReleaseDir)) {
        throw new Error(
          [
            `Expected Windows arm64 build output at ${expectedReleaseDir}, but Flutter produced x64 output at ${x64ReleaseDir}.`,
            "The current Flutter toolchain fell back to windows-x64, so an arm64 Store MSIX cannot be created from this build.",
          ].join(" "),
        );
      }
    }

    throw new Error(`Expected Windows build output at ${expectedReleaseDir}, but it was not created.`);
  }
